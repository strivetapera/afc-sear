...
import { createContentItemRecord, getPublicEventsFeed, getPublicLiveWebcast, getPublicNewsFeed, getPublishedContentBySlug, getStructuredPageBySlug, listContentItemRecords, getContentItemById, updateContentItem, publishContentItem, } from './repositories/content-repository';
...
import { getEventBySlug, listEventRegistrations, listPublicEvents, registerForEvent, recordCheckIn, createEvent, updateEvent, listAdminEvents, addSchedule, addTicketType, } from './repositories/event-repository';
import { listAnnouncements, createAnnouncement, updateAnnouncement, listPublicAnnouncements, listCampaigns, createCampaign, listMessageTemplates, createMessageTemplate, listDeliveryLogs, } from './repositories/communication-repository';
import { listFunds, createFund, listPayments, listDonations, createDonationIntent, capturePayment, } from './repositories/finance-repository';
...
export function createApp() {
    ...
    app.after(() => {
        ...
        // ─── Identity ────────────────────────────────────────────────────────────
        app.post('/api/v1/auth/sign-in', { schema: { body: schemas.SignInSchema } }, async (request, reply) => {
        ...
        app.patch('/api/v1/admin/content-items/:id', { preHandler: [app.authenticate, app.requireRole(ADMIN_ROLE)] }, async (request) => {
            ...
            const result = await updateContentItem(request.params.id, request.body);
            await writeAuditLog({
                actorUserId: request.user?.id,
        ...
        app.post('/api/v1/admin/campaigns/:id/send', { preHandler: [app.authenticate, app.requireRole(ADMIN_ROLE)] }, async (request) => {
            const { id } = request.params;
            const { db: prisma } = await import('./repositories/prisma').then((m) => ({ db: m.getPrismaClient() }));
            const campaign = await prisma.campaign.update({
                where: { id },
