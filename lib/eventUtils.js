...
function calculateNextOccurrence(recurrence, now) {
  ...
  try {
    ...
    const nextDate = new Date(now); // Clone 'now' to avoid modifying original
...
function formatDuration(durationMinutes) {
  if (!durationMinutes || Number.isNaN(durationMinutes)) return 'Duration varies';
  if (durationMinutes < 60) return `${durationMinutes} minutes`;
  if (durationMinutes % 60 === 0) return `${durationMinutes / 60} hour${durationMinutes === 60 ? '' : 's'}`;
...
export async function getOnlineEventsStructured(limit = null) {
  const upcomingEvents = await getUpcomingEvents(null);
  const onlineEvents = upcomingEvents
    .filter((event) => event.link)
    .map((event) => ({
      id: event.id,
      title: event.title,
      occurrenceLabel: formatEventDate(event.calculatedNextOccurrence, event.recurrence),
      durationLabel: formatDuration(event.recurrence?.durationMinutes),
      joinUrl: event.link,
      description: stripJoinDetails(event.description),
      providerLabel: event.link.includes('zoom.us')
        ? 'Zoom'
        : event.link.includes('youtube')
          ? 'YouTube'
          : event.link.includes('facebook')
            ? 'Facebook'
            : 'Online',
    }));

  return limit ? onlineEvents.slice(0, limit) : onlineEvents;
}
